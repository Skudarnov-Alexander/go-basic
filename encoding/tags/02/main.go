...
func Validate(obj interface{}) bool {
    ...
    for i := 0; i < objType.NumField(); i++ {
        ...
        if v, ok := vobj.Field(i).Interface().(int); ok {
			...
			tagLimitValue, ok := objType.Field(i).Tag.Lookup("limit")
			if !ok {
				continue
			}
			fmt.Printf("Значение тега 'limit' поля '%s' равно: %s\n",objType.Field(i).Name, tagLimitValue)
			
			limits := strings.Split(tagLimitValue, ",")
